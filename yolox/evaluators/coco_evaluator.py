...
class COCOEvaluator:
    ...
    def evaluate_prediction(self, data_dict, statistics):
        ...
        if len(data_dict) > 0:
            ...
            # Filter invalid predictions first
            valid_data_dict = []
            ...
            # FIXED: Proper tempfile handling
            cocoDt = None
            temp_file_path = None
            
            try:
                if self.testdev:
                    json.dump(valid_data_dict, open("./yolox_testdev_2017.json", "w"))
                    ...
                    try:
                        ...
                        cocoDt = cocoGt.loadRes(temp_file_path)
                    except Exception as e:
                        logger.error(f"Error writing to temp file: {e}")
                        # Fallback: try writing to current directory
                        fallback_path = "temp_predictions.json"
                        with open(fallback_path, "w") as f:
                            json.dump(valid_data_dict, f)
                        cocoDt = cocoGt.loadRes(fallback_path)
            ...
            if cocoDt is None:
                logger.error("Failed to create COCO detection object")
                return 0, 0, info
